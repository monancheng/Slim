...
namespace DoozyUI
{
    ...
    public class SceneLoader : Singleton<SceneLoader>
    {
        ...
        private static void CreateCustomGameObject(MenuCommand menuCommand)
        {
            ...
            {
                Debug.LogError(
                    "[DoozyUI] The DoozyUI system was not found in the scene. Please add it before trying to create a Scene Loader.");
                return;
            }
            ...
            var go = new GameObject("Scene Loader");
        ...
        {
#if dUI_EnergyBarToolkit // If if we are loading a level and we have energyBars linkd then we update their values
        ...
        private void RegisterSceneLoader()
        {
            ...
            else
            {
                ...
                Debug.LogWarning(
                    "[DoozyUI] An instance of a SceneLoader is already registared to the UIManager. There should never be more than 1 SceneLoader in the Hierarcy! This gameObject has been renamed to 'SceneLoader_DUPLICATE'. Look for it in the Hierarchy, while 'Play Mode', then select it, exit 'Play Mode' and then delete it.");
            }
        }

        private void UnregisterSceneLoader()
        {
            if (UIManager.sceneLoader != null)
        ...
        public void OnGameEvent(GameEventMessage m)
        {
            ...
            else if (m.command.Contains(command_LoadLevel)
            ) //SHORTCUT VARIANT - we just call LoadLevel_{LevelNumber} and we load additive async the level data
            {
                sceneName = levelSceneName + m.command.Split('_')[1];
                async = SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Additive);
            }
            else if (m.command.Contains(command_UnloadLevel)
            ) ///SHORTCUT VARIANT - we just call UnloadLevel_{LevelNumber} and we unload the level data
            {
                sceneName = levelSceneName + m.command.Split('_')[1];
