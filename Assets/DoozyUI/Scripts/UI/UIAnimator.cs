...
namespace DoozyUI
{
    public class UIAnimator : MonoBehaviour
    {
        ...
        /// </summary>
        public static void ResetRectTransform(RectTransform rectTransform, InitialData initialData,
        ...
        public class MoveIn
        {
            ...
            /// <summary>
            ///     Use this if you need to adjust the target position. You add or subtract (if the number is negative) values to the
        ...
        public class MoveOut
        {
            ...
            /// <summary>
            ///     Use this if you need to adjust the target position. You add or substract (if the number is negative) values to the
            ///     position of the target location
            /// </summary>
            public Vector3 positionAdjustment = Vector3.zero;
        ...
        ///     To play a sound and check if the sound is on in UIManager call UIAnimator.PlaySound(soundName,
        ///     UIManager.Instance.soundOn);
        /// </summary>
        ...
        {
            if (string.IsNullOrEmpty(soundName) || soundOn == false ||
                soundName.Equals(UIManager.DEFAULT_SOUND_NAME)) return;
            var soundOutput = SoundOutput.AudioSource;
        ...
        /// </summary>
        public static void DoMoveIn(MoveIn moveIn, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
            if (!moveIn.enabled) return;
            var parent =
                rectTransform.parent
                    .GetComponent<RectTransform>(); //We need to do this check because when we Instantiate a notification we need to use the uiContainer if the parent is null.
            if (parent == null) parent = UIManager.GetUiContainer.GetComponent<RectTransform>();
            var targetPosition = initialData.startAnchoredPosition3D;
#if UNITY_5_3 == false
            var tempCanvas = rectTransform.GetComponent<Canvas>();
#endif
...
#else
            if (tempCanvas == null
            ) //this might be a button or an UIElement that does not have a Canvas component (this should not happen)
                rootCanvas = rectTransform.root.GetComponentInChildren<Canvas>();
            else
                rootCanvas = tempCanvas.rootCanvas;
#endif
            var rootCanvasRect = rootCanvas.GetComponent<RectTransform>().rect;
            var xOffset = rootCanvasRect.width / 2 + rectTransform.rect.width * rectTransform.pivot.x;
            var yOffset = rootCanvasRect.height / 2 + rectTransform.rect.height * rectTransform.pivot.y;
            ...
            switch (moveIn.moveFrom)
            {
                ...
                default:
                    Debug.LogWarning(
                        "[Doozy] This should not happen! DoMoveIn in UIAnimator went to the default setting!");
                    return;
            }
        ...
        /// </summary>
        public static void DoMoveInAnimation(Vector3 position, MoveIn moveIn, RectTransform rectTransform,
            InitialData initialData, bool instantAction)
        {
            if (instantAction)
            {
                rectTransform.anchoredPosition = initialData.startAnchoredPosition3D;
        ...
        /// </summary>
        public static void DoRotationIn(RotationIn rotationIn, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
        ...
        /// </summary>
        public static void DoScaleIn(ScaleIn scaleIn, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
            if (!scaleIn.enabled) return;
            if (instantAction)
            {
                rectTransform.localScale = scaleIn.scaleBegin;
                StartLoopAnimations(rectTransform);
                return;
            }
            rectTransform
        ...
        /// </summary>
        public static void DoFadeIn(FadeIn fadeIn, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
        ...
        /// <summary>
        ///     Stops the IN animations on a rectTransform. This is used before any OUT animations to avoid calling a hide method
        ///     while a show method is still playing (without this, unexpected behaviour can occur)
        /// </summary>
        ...
        /// <summary>
        ///     This initialises and plays (if set to autoStart) the idle animation Move Loop for a RectTransform with a
        ///     PanelController component added to it.
        /// </summary>
        public static void DoMoveLoop(MoveLoop moveLoop, RectTransform rectTransform, InitialData initialData)
        {
            ...
            var animBeginPosition = new Vector3(initialData.startAnchoredPosition3D.x - moveLoop.movement.x,
                initialData.startAnchoredPosition3D.y - moveLoop.movement.y,
                initialData.startAnchoredPosition3D.z - moveLoop.movement.z);

            var animEndPosition = new Vector3(initialData.startAnchoredPosition3D.x + moveLoop.movement.x,
                initialData.startAnchoredPosition3D.y + moveLoop.movement.y,
                initialData.startAnchoredPosition3D.z + moveLoop.movement.z);
        ...
        /// <summary>
        ///     This initialises and plays (if set to autoStart) the idle animation Rotation Loop for a RectTransform with a
        ///     PanelController component added to it.
        /// </summary>
        public static void DoRotationLoop(RotationLoop rotationLoop, RectTransform rectTransform,
            InitialData initialData)
        {
            if (!rotationLoop.enabled) return;
            var animBeginRotation = new Vector3(initialData.startRotation.x - rotationLoop.rotation.x,
            ...
            var animEndRotation = new Vector3(initialData.startRotation.x + rotationLoop.rotation.x,
                initialData.startRotation.y + rotationLoop.rotation.y,
        ...
        /// <summary>
        ///     This initialises and plays (if set to autoStart) the idle animation Scale Loop for a RectTransform with a
        ///     PanelController component added to it.
        /// </summary>
        ...
        /// <summary>
        ///     This initialises and plays (if set to autoStart) the idle animation Fade Loop for a RectTransform with a
        ///     PanelController component added to it.
        /// </summary>
        public static void DoFadeLoop(FadeLoop fadeLoop, RectTransform rectTransform, InitialData initialData,
            AnimationTarget animationTarget = AnimationTarget.UIElement)
        {
            ...
            switch (animationTarget)
            {
                case AnimationTarget.UIElement:
                    ...
                    break; //we do this so that the UIElement ignores the clicks
                case AnimationTarget.UIButton:
                    canvasGroup.blocksRaycasts = true;
                    break; //we do this so that we can click on the button
            }
            canvasGroup
        ...
        {
            var uiElement = rectTransform.GetComponent<UIElement>();
            if (uiElement == null)
            {
                Debug.LogWarning("[Doozy] " + rectTransform.name +
                                 " does not have a UIElement Component attached and you are trying to start an idle animation");
                return;
            }
            var id = rectTransform.GetInstanceID();
        ...
        public static void StopLoopAnimations(RectTransform rectTransform, InitialData initialData)
        {
            ...
            {
                var layout = rectTransform.parent.GetComponent<LayoutGroup>();
        ...
        /// </summary>
        public static void DoMoveOut(MoveOut moveOut, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
            ...
            switch (moveOut.moveTo)
            {
                ...
                default:
                    Debug.LogWarning(
                        "[Doozy] This should not happen! DoMoveOut in UIAnimator went to the default setting!");
                    return;
            }
        ...
        /// </summary>
        public static void DoMoveOutAnimation(Vector2 position, MoveOut moveOut, RectTransform rectTransform,
            InitialData initialData, bool instantAction)
        {
            if (instantAction)
            {
        ...
        /// </summary>
        public static void DoRotationOut(RotationOut rotationOut, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
        ...
        /// </summary>
        public static void DoScaleOut(ScaleOut scaleOut, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
        ...
        /// </summary>
        public static void DoFadeOut(FadeOut fadeOut, RectTransform rectTransform, InitialData initialData,
            bool instantAction)
        {
        ...
        /// <summary>
        ///     Stops the OUT animations on a rectTransform. This is used before any IN animations to avoid calling a show method
        ///     while a hide method is still playing (without this, unexpected behaviour can occur)
        /// </summary>
        ...
        public static void StartOnClickAnimations(RectTransform rectTransform, InitialData initialData,
            UIAnimationManager.OnClickAnimations onClickAnimSettings)
        {
            if (onClickAnimSettings.punchPositionEnabled)
                rectTransform.DOPunchAnchorPos(onClickAnimSettings.punchPositionPunch,
                        onClickAnimSettings.punchPositionDuration, onClickAnimSettings.punchPositionVibrato,
                        onClickAnimSettings.punchPositionElasticity, onClickAnimSettings.punchPositionSnapping)
            ...
            if (onClickAnimSettings.punchRotationEnabled)
                rectTransform.DOPunchRotation(onClickAnimSettings.punchRotationPunch,
                        onClickAnimSettings.punchRotationDuration, onClickAnimSettings.punchRotationVibrato,
                        onClickAnimSettings.punchPositionElasticity)
            ...
            if (onClickAnimSettings.punchScaleEnabled)
                rectTransform.DOPunchScale(onClickAnimSettings.punchScalePunch, onClickAnimSettings.punchScaleDuration,
                        onClickAnimSettings.punchScaleVibrato, onClickAnimSettings.punchScaleElasticity)
        ...
        /// </summary>
        public static void StartButtonLoopsAnimations(RectTransform rectTransform, InitialData initialData,
            UIAnimationManager.ButtonLoopsAnimations animationSettings)
        {
            if (rectTransform.GetComponent<UIButton>() == null)
            {
                Debug.LogWarning("[Doozy] " + rectTransform.name +
                                 " does not have a UIButton Component attached and you are trying to start a normal state animation");
                return;
            }
            var id = rectTransform.GetInstanceID();
