...
namespace DoozyUI
{
    ...
    public class UINotification : MonoBehaviour
    {
        ...
        private static void CreateCustomGameObject(MenuCommand menuCommand)
        {
            ...
            {
                Debug.LogError(
                    "[DoozyUI] The DoozyUI system was not found in the scene. Please add it before trying to create a UI Notification.");
                return;
            }

            var notification = new GameObject("New UINotification");
            ...
            var notificationContainer = new GameObject("NotificationContainer");
            notificationContainer.transform.parent = notification.transform;
            notificationContainer.AddComponent<RectTransform>();
            notificationContainer.GetComponent<RectTransform>().position =
                notification.GetComponent<RectTransform>().position;
            ...
            notification.GetComponent<UINotification>().notificationContainer =
                notificationContainer.GetComponent<UIElement>();
        ...
        private void Awake()
        {
            ...
            notificationName =
                "Notification" + " [" + gameObject.GetInstanceID() +
                "]"; //we generate a unique name for this notification UIElement (we need it to be able to hide it when we press one of it's buttons)
            notificationContainer.elementName = notificationName;
            notificationContainer.autoRegister =
                false; //we want to register this element to the registry with the auto generated name (notifiacationName)
            notificationContainer.animateAtStart = true;

            if (overlay != null)
            {
                overlay.elementName =
                    notificationName; //we use the same elementName so it will get hidden along with the main uiElement panel
                overlay.autoRegister =
                    false; //we want this notification to register this element to the registry with the auto generated name (notifiacationName)
            ...
            if (specialElements != null && specialElements.Length > 0)
                ...
                {
                    specialElements[i].elementName =
                        notificationName; //we use the same elementName so it will get hidden along with the main uiElement panel
                    specialElements[i].autoRegister =
                        false; //we want this notification to register this element to the registry with the auto generated name (notifiacationName)
            ...
            if (effects != null && effects.Length > 0)
                for (var i = 0; i < effects.Length; i++)
                    ...
                    {
                        effects[i].targetUIElement.elementName =
                            notificationName; //we use the same elementName so it will get hidden along with the main uiElement panel
                        effects[i].autoRegister =
                            false; //we want this notification to register this effect to the registry with the auto generated name (notifiacationName)
        ...
        private void OnEnable()
        {
            if (notificationContainer == null)
            {
                Debug.Log("[DoozyUI] The UINotification on [" + gameObject.name +
                          "] gameObject is disabled. It will not work because you didn't link a notification container. This should be a child of the notification gamObject and it should have a UIElement on it. Also, the notification container should have at least one IN and one OUT animations enabled.");
                return;
            }
        ...
        private void Update()
        {
            if (listenForBackButton && Input.GetKeyDown(KeyCode.Escape)
            ) //The listener for the 'Back' button event; we do this because we do not want to change the UI state while a notification is on screen (we disabled the back button from the UI Manager and the UI Notifications needs to listen for it now)
                if (closeOnClick
                ) //only if closeOnClick has been enabled (a closeOnClick button has been assigned) we close the notification
                    BackButtonEvent();
        }
        ...
        public void ShowNotification(NotificationData ndata)
        {
            ...
            ) //in case we have a child that is not on the proper layer, we set it here so it shows up in the UI camera
                t.gameObject.layer = gameObject.layer;

            notificationData = ndata; //we save this data to use it if we need to unregister from the Notification Queue

            if (icon != null && ndata.icon != null
            ) //if this notification has an icon slot and the show notification passed a new icon, we update it
                icon.sprite = ndata.icon;

            if (ndata.buttonTexts == null || ndata.buttonTexts.Length == 0)
                closeOnClick =
                    true; //if there are no button texts we let the user close this notification just by ckicking it

            if (ndata.buttonNames == null || ndata.buttonNames.Length == 0)
                closeOnClick =
                    true; //if there are no button names we let the user close this notification just by ckicking it

            if (closeButton != null && closeOnClick
            ) //if we linked the closeOnClickButton, we configure it to close the notification window on click
            {
                var b = closeButton.GetComponent<UIButton>();
                var onClickDelay = 0f;
                if (b != null
                ) //we check if we have an UIButton attached (we do this so that if the button has an onClick animation, we show it and after that we hide the notification)
                    onClickDelay =
                        b.GetOnClickAnimationsDuration; //this creates a more pleasent user experience (UX) by letthing the OnClick animation play before hiding the notification
            ...
            if (ndata.lifetime > 0
            ) //We look for the lifetime (if it's -1 we do not auto hide the notification. We wait for the player to hit a button.
            {
                ...
                StartCoroutine(DestroyAfterTime(GetInAnimationsTimeAndDelay() + ndata.lifetime +
                                                GetOutAnimationsTimeAndDelay()));
            }

            if (UIManager.usesTMPro
            ) //If we are using the TextMeshPro plugin we will look for TextMeshProUGUI component otherwise we look for the native Text component
            {
#if dUI_TextMeshPro
                if (this.title != null)
            ...
            if (buttons != null && ndata.buttonNames != null
            ) //If this notification prefab has buttons and buttonNames is not null (those are the buttonNames we are listening for) we start adding the buttons
                for (var i = 0; i < buttons.Length; i++)
                {
                    var index = i;
                    buttons[i].GetComponent<Button>().onClick.AddListener(() =>
                    {
                        if (ndata.buttonCallback != null && index < ndata.buttonCallback.Length &&
                            ndata.buttonCallback[index] != null)
                            ndata.buttonCallback[index].Invoke();
                        StartCoroutine(HideNotification(0.2f));
                        StartCoroutine(
                            DestroyAfterTime(GetOutAnimationsTimeAndDelay() +
                                             0.2f)); //We destroy this notification after the Out animation finished
                    });

                    if (ndata.buttonNames.Length > i && string.IsNullOrEmpty(ndata.buttonNames[i]) == false
                    ) //If we have a buttonName we make the button active and set the buttonName to the UIButton compoenent
                    {
                        buttons[i].gameObject.SetActive(true);
                        buttons[i].buttonName = ndata.buttonNames[i]; //We set the buttonName

                        if (ndata.buttonTexts != null
                        ) //We might not have a text for the button (it might be an image or an icon) so we check if we wanted a text on it
...
ndata.buttonTexts[i];
                    ...
                    else
                    {
                        buttons[i].gameObject
                            .SetActive(
                                false); //if we still have unused buttons on this notification prefab, we hide them
                    }
                }
        ...
        {
            if (notificationContainer == null)
                Debug.LogError("[DoozyUI] The notification [" + gameObject.name +
                               "] does not have an UIElement linked to the notification field. This should not happen and you will get unexpected results. To fix this, link a child UIElement as the notification's body.");
            else
            ...
            if (specialElements != null && specialElements.Length > 0)
                for (var i = 0; i < specialElements.Length; i++)
                    if (specialElements[i] != null)
                        ...
                        Debug.LogWarning("[DoozyUI] The notification [" + gameObject.name +
                                         "] has unassigned array slots for the Special Elements. To fix this, just remove the unused slots in the array or assign UIElements to them.");
            ...
            if (effects != null && effects.Length > 0)
                for (var i = 0; i < effects.Length; i++)
                    ...
                    {
                        Debug.LogWarning("[DoozyUI] The notification [" + gameObject.name +
                                         "] has unassigned array slots for the Effects. To fix this, just remove the unused slots in the array or assign UIEffects to them.");
                    }
        }
        ...
        private void UnregisterFromUIManager()
        {
            if (notificationContainer == null)
                Debug.LogError("[DoozyUI] The notification [" + gameObject.name +
                               "] does not have an UIElement linked to the notification field. This should not happen and you will get unexpected results.");
            else
        ...
        private void UnregisterFromNotificationQueue(NotificationData nData)
        {
        ...
        private float GetInAnimationsTimeAndDelay()
        {
            ...
            var inAnimationsTimeAndDelay = Mathf.Max(moveInTimeAndDelay, rotationInTimeAndDelay, scaleInTimeAndDelay,
                fadeInTimeAndDelay);
        ...
        private float GetOutAnimationsTimeAndDelay()
        {
            ...
            if (notificationContainer.rotationOut.enabled)
                rotationOutTimeAndDelay =
                    notificationContainer.rotationOut.time + notificationContainer.rotationOut.delay;
            ...
            var outAnimationsTimeAndDelay = Mathf.Max(moveOutTimeAndDelay, rotationOutTimeAndDelay,
                scaleOutTimeAndDelay, fadeOutTimeAndDelay);
        ...
        public class NotificationData
        {
            ...
            /// <summary>
            ///     If the notification has buttons, these are the buttonNames that will be sent on Button Click. If there are 3
            ///     buttons available and you enter only 2 buttonNames, only those 2 buttons will be visible and active (the 3rd will
            ///     not appear, nor work).
            /// </summary>
            public string[] buttonNames = defaultButtonNames;

            /// <summary>
            ///     If the notification has buttons and those buttons have a Text or a TextMeshProUGUI compoment attached to them or
            ///     one of their children, then these are the button text that will appear on the buttons.
            ///     If there are 3 buttons available and active, and you enter the button text for only 2 of them, only the first 2
            ///     buttons well have a text, and the third will have nothing.
        ...
        public GameObject title; //the title of the notification
        public GameObject message; //the message of the notification
        public Image icon; //the icon of the notification
        public UIButton[] buttons; //the buttons on the notification
        public Button closeButton; //this is a button that covers the entire notification and closes it on click

        public UIElement[] specialElements
            ; //if there are special elements that need to be animated they need to be linked here (like stars, banners, flags, icons...)
        ...
        private string notificationName = string.Empty
            ; //this is used to auto-generate a uiElement name (we use it to hide this notification when a one of it's buttons is clicked)
