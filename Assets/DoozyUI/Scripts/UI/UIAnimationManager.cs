...
namespace DoozyUI
{
    ...
    public class UIAnimationManager : MonoBehaviour
    {
        ...
        public void SavePreset(string presetName, AnimationType animationType,
            ButtonLoopType buttonLoopType = ButtonLoopType.None)
        {
            ...
            var adjustedFolderPath = GetUIAnimationsFolderPath + animationType;
            var presetFileName = presetName + "_" + animationType + fileNameExtension;
            var filePath = adjustedFolderPath + "/" + presetFileName;

            switch (animationType)
            {
                case AnimationType.IN:
                    var inAnimations = new InAnimations();
                ...
                case AnimationType.LOOP:
                    var loopAnimations = new LoopAnimations();
                ...
                case AnimationType.OUT:
                    var outAnimations = new OutAnimations();
                ...
                case AnimationType.OnClick:
                    var onClickAnimationSettings = new OnClickAnimations();
                ...
                case AnimationType.ButtonLoops:
                    switch (buttonLoopType)
                    {
                        case ButtonLoopType.Normal:
                            var normalAnimationSettings = new ButtonLoopsAnimations();
                        ...
                        case ButtonLoopType.Highlighted:
                            var highlightedAnimationSettings = new ButtonLoopsAnimations();
                            ...
                            FileHelper.writeObjectToFile(filePath, highlightedAnimationSettings,
                                FileHelper.SerializeXML);
                        ...
                        case ButtonLoopType.None: //we should not be able to get here
                            Debug.Log("[DoozyUI] [UIButton] You are trying to save a preset named '" + presetName +
                                      "'. Something went wrong and the preset was not saved. This should not happen.");
                            break;
        ...
        public void LoadPreset(string presetName, AnimationType animationType,
            ButtonLoopType buttonLoopType = ButtonLoopType.None)
        {
            var adjustedFolderPath = GetUIAnimationsFolderPath + animationType;
            var presetFileName = presetName + "_" + animationType + fileNameExtension;
            ...
            switch (animationType)
            {
                case AnimationType.IN:
                    ...
                    GetUIElement.SetInAnimations =
                        FileHelper.readObjectFile(filePath, FileHelper.DeserializeXML<InAnimations>);
                    for (var i = 0; i < GetUIElement.inAnimationsPresetNames.Length; i++)
                        if (GetUIElement.GetInAnimations.inAnimationsPresetName.Equals(
                            GetUIElement.inAnimationsPresetNames[i]))
                        {
                ...
                case AnimationType.LOOP:
                    ...
                    GetUIElement.SetLoopAnimations =
                        FileHelper.readObjectFile(filePath, FileHelper.DeserializeXML<LoopAnimations>);
                    for (var i = 0; i < GetUIElement.loopAnimationsPresetNames.Length; i++)
                        if (GetUIElement.GetLoopAnimations.loopAnimationsPresetName.Equals(
                            GetUIElement.loopAnimationsPresetNames[i]))
                        {
                ...
                case AnimationType.OUT:
                    ...
                    GetUIElement.SetOutAnimations =
                        FileHelper.readObjectFile(filePath, FileHelper.DeserializeXML<OutAnimations>);
                    for (var i = 0; i < GetUIElement.outAnimationsPresetNames.Length; i++)
                        if (GetUIElement.GetOutAnimations.outAnimationsPresetName.Equals(
                            GetUIElement.outAnimationsPresetNames[i]))
                        {
                ...
                case AnimationType.OnClick:
                    GetUIButton.onClickAnimationSettings =
                        FileHelper.readObjectFile(filePath, FileHelper.DeserializeXML<OnClickAnimations>);
                    GetUIButton.onClickAnimationsPresetName = presetName;
                    for (var i = 0; i < GetUIButton.onClickAnimationsPresetNames.Length; i++)
                ...
                case AnimationType.ButtonLoops:
                    ...
                    {
                        case ButtonLoopType.Normal:
                            GetUIButton.normalAnimationSettings = FileHelper.readObjectFile(filePath,
                                FileHelper.DeserializeXML<ButtonLoopsAnimations>);
                            GetUIButton.normalAnimationsPresetName = presetName;
                            for (var i = 0; i < GetUIButton.buttonLoopsAnimationsPresetNames.Length; i++)
                                if (GetUIButton.normalAnimationsPresetName.Equals(
                                    GetUIButton.buttonLoopsAnimationsPresetNames[i]))
                                {
                        ...
                        case ButtonLoopType.Highlighted:
                            GetUIButton.highlightedAnimationSettings = FileHelper.readObjectFile(filePath,
                                FileHelper.DeserializeXML<ButtonLoopsAnimations>);
                            GetUIButton.highlightedAnimationsPresetName = presetName;
                            for (var i = 0; i < GetUIButton.buttonLoopsAnimationsPresetNames.Length; i++)
                                if (GetUIButton.highlightedAnimationsPresetName.Equals(
                                    GetUIButton.buttonLoopsAnimationsPresetNames[i]))
                                {
                        ...
                        case ButtonLoopType.None: //we should not be able to get here
                            Debug.Log("[DoozyUI] [UIButton] You are trying to load a preset named '" + presetName +
                                      "'. Something went wrong and the preset was not loaded. This should not happen.");
                            break;
        ...
        {
            var adjustedFolderPath = GetUIAnimationsFolderPath + animationType;
            var presetFilesInfo = Directory.GetFiles(adjustedFolderPath, "*" + fileNameExtension);
            var fileCount = presetFilesInfo.Length;
            ...
            switch (animationType)
            {
                case AnimationType.IN:
                    if (fileCount > 0)
                    {
                        ...
                        fileCount = 0;
                        for (var i = 0; i < presetFilesInfo.Length; i++)
                        {
                            GetUIElement.inAnimationsPresetNames[fileCount] = FileHelper
                                .readObjectFile(presetFilesInfo[i], FileHelper.DeserializeXML<InAnimations>)
                                .inAnimationsPresetName;
                            fileCount++;
                        }
                    }
                    break;
                ...
                case AnimationType.LOOP:
                    if (fileCount > 0)
                    {
                        ...
                        for (var i = 0; i < presetFilesInfo.Length; i++)
                        {
                            GetUIElement.loopAnimationsPresetNames[fileCount] = FileHelper
                                .readObjectFile(presetFilesInfo[i], FileHelper.DeserializeXML<LoopAnimations>)
                                .loopAnimationsPresetName;
                            fileCount++;
                        }
                    }
                    break;
                ...
                case AnimationType.OUT:
                    if (fileCount > 0)
                    {
                        ...
                        for (var i = 0; i < presetFilesInfo.Length; i++)
                        {
                            GetUIElement.outAnimationsPresetNames[fileCount] = FileHelper
                                .readObjectFile(presetFilesInfo[i], FileHelper.DeserializeXML<OutAnimations>)
                                .outAnimationsPresetName;
                            fileCount++;
                        }
                    }
                    break;
                ...
                case AnimationType.OnClick:
                {
                    if (fileCount > 0)
                    {
                        ...
                        for (var i = 0; i < presetFilesInfo.Length; i++)
                        {
                            GetUIButton.onClickAnimationsPresetNames[fileCount] = FileHelper
                                .readObjectFile(presetFilesInfo[i], FileHelper.DeserializeXML<OnClickAnimations>)
                                .onClickAnimationsPresetName;
                            fileCount++;
                        }
                    }
                }
                ...
                case AnimationType.ButtonLoops:
                {
                    if (fileCount > 0)
                    {
                        ...
                        for (var i = 0; i < presetFilesInfo.Length; i++)
                        {
                            GetUIButton.buttonLoopsAnimationsPresetNames[fileCount] = FileHelper
                                .readObjectFile(presetFilesInfo[i], FileHelper.DeserializeXML<ButtonLoopsAnimations>)
                                .animationsPresetName;
                            fileCount++;
                        }
                    }
                }
        ...
        public void DeletePreset(string presetName, AnimationType animationType,
            ButtonLoopType buttonLoopType = ButtonLoopType.None)
        {
            if (presetName.Equals(DEFAULT_PRESET_NAME))
            {
        ...
        public class OnClickAnimations
        {
            ...
            public Vector2 punchPositionPunch = new Vector2(0, 20)
            ...
            public float punchPositionElasticity = 0.5f
            ...
            public Vector3 punchRotationPunch = new Vector3(0, 0, 30)
            ...
            public float punchRotationElasticity = 0.5f
            ...
            public Vector3 punchScalePunch = new Vector3(0.2f, 0.2f, 0)
            ...
            public float punchScaleElasticity = 0.5f
        ...
        private readonly string fileNameExtension = ".xml";
