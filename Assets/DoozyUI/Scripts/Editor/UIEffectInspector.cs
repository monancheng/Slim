...
public class UIEffectInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (uiEffect.targetUIElement == null && uiEffect.showHelp)
        {
            ...
            EditorGUILayout.HelpBox(
                "Target UIElement: this effect will be linked to the show and hide events of the target element.",
                MessageType.None);
            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.Doozy);
        }
        ...
        else
        {
            ...
            sp_playOnAwake.boolValue =
                EditorGUILayout.ToggleLeft("play on awake", sp_playOnAwake.boolValue, GUILayout.Width(160));
            sp_stopInstantly.boolValue = EditorGUILayout.ToggleLeft("stop instantly on hide",
                sp_stopInstantly.boolValue, GUILayout.Width(160));
            ...
            if (uiEffect.showHelp)
            {
                DoozyUIHelper.ResetColors();
                EditorGUILayout.HelpBox(
                    "play on awake: just like the UIElement's 'start hidden' option, the play on awake should be used if this effect will be visible when the scene strts. Otherwise you should let this option unchecked.",
                    MessageType.None);
                EditorGUILayout.HelpBox(
            ...
            if (uiEffect.showHelp)
            {
                DoozyUIHelper.ResetColors();
                EditorGUILayout.HelpBox(
                    "Update UI Effect: this option helps you adjust/update the sorting order on the fly whenever you want.",
                    MessageType.None);
                DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.Doozy);
            }
        }
    ...
    private SerializedProperty sp_targetUIElement;
    private SerializedProperty sp_playOnAwake;
    private SerializedProperty sp_startDelay;
    private SerializedProperty sp_stopInstantly;
    private SerializedProperty sp_effectPosition;
    private SerializedProperty sp_sortingOrderStep;
    ...
    private UIEffect.EffectPosition effectPosition;
