...
public class UITriggerInspector : Editor
{
    ...
    private void ShowTriggerEvent()
    {
        ...
        if (uiTrigger.gameEvents != null) //we check if the gameEvents list has any items in it
            for (var i = 0; i < uiTrigger.gameEvents.Count; i++) //we show the list of elements
            {
    ...
    private void ShowHelpInstructions()
    {
        if (isTriggerEnabled)
        {
            ...
            if (sp_triggerOnGameEvent.boolValue)
                if (sp_dispatchAll.boolValue)
                {
                    ...
                    if (uiTrigger.showHelp)
                    {
                        ...
                        EditorGUILayout.HelpBox(
                            "Because you selected the Dispatch ALL option, you should be aware that in order for it to work as intended you must call a function that has a string as a paramater and...",
                            MessageType.Info);
                        EditorGUILayout.HelpBox(
                            "It is VERY IMPORTANT that you select, in the 'On Trigger Event (String)' section, from the right drop down list, a function that is located in the 'Dynamic string' section (on top) and not the 'Static parameters' section (on bottom). See the documentation for mode details.",
                            MessageType.Warning);
                    }
                }
                else
                {
                    EditorGUILayout.HelpBox("Dispatching the [" + sp_gameEvent.stringValue + "] Game Event...",
                        MessageType.None);
                }
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Dispatching Button Clicks for the [" + sp_buttonName.stringValue + "] button...",
                        MessageType.None);
                }
        }
        else if (uiTrigger.showHelp)
        {
            ...
            if (sp_triggerOnGameEvent.boolValue)
                EditorGUILayout.HelpBox(
                    "Enter the game event you want this trigger to listen for. Or select Dispatch ALL to send all game events.",
                    MessageType.None);
            else if (sp_triggerOnButtonClick.boolValue)
                EditorGUILayout.HelpBox(
                    "Select the button name you want this trigger to listen for. Or select Dispatch ALL to send all button clicks.",
                    MessageType.None);
            else
                EditorGUILayout.HelpBox(
                    "You can activate this trigger by listening for either a game event or a button name.",
                    MessageType.None);
        }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (sp_triggerOnGameEvent.boolValue == false && sp_triggerOnButtonClick.boolValue == false)
        {
            ...
            sp_triggerOnGameEvent.boolValue =
                EditorGUILayout.ToggleLeft("Listen for Game Events", sp_triggerOnGameEvent.boolValue);
            ...
            sp_triggerOnButtonClick.boolValue =
                EditorGUILayout.ToggleLeft("Listen for Button Clicks", sp_triggerOnButtonClick.boolValue);
        ...
        else
        {
            if (sp_triggerOnGameEvent.boolValue)
            {
                ...
                sp_triggerOnGameEvent.boolValue = EditorGUILayout.ToggleLeft("Listen for Game Events",
                    sp_triggerOnGameEvent.boolValue, GUILayout.Width(180));
                ...
                sp_dispatchAll.boolValue = EditorGUILayout.ToggleLeft("Dispatch All Game Events",
                    sp_dispatchAll.boolValue, GUILayout.Width(180));
                ...
                if (sp_dispatchAll.boolValue == false)
                {
                    ...
                    sp_gameEvent.stringValue =
                        EditorGUILayout.TextField(sp_gameEvent.stringValue, GUILayout.Width(320));
                    GUILayout.FlexibleSpace();
                    EditorGUILayout.EndHorizontal();
            ...
            else if (sp_triggerOnButtonClick.boolValue)
            {
                ...
                sp_triggerOnButtonClick.boolValue = EditorGUILayout.ToggleLeft("Listen for Button Clicks",
                    sp_triggerOnButtonClick.boolValue, GUILayout.Width(180));
                ...
                sp_dispatchAll.boolValue = EditorGUILayout.ToggleLeft("Dispatch All Button Clicks",
                    sp_dispatchAll.boolValue, GUILayout.Width(180));
                ...
                if (sp_dispatchAll.boolValue == false)
                {
                    ...
                    ) //we have a button name that is not in the database; we reset this value
                    ...
                    buttonNameCurrentIndex =
                        EditorGUILayout.Popup(buttonNameCurrentIndex, buttonNames, GUILayout.Width(320));
                    GUILayout.FlexibleSpace();
                    EditorGUILayout.EndHorizontal();
    ...
    private SerializedProperty sp_triggerOnGameEvent;
    private SerializedProperty sp_gameEvent;

    private SerializedProperty sp_triggerOnButtonClick;
    private SerializedProperty sp_buttonName;

    private SerializedProperty sp_dispatchAll;

    private SerializedProperty sp_onTriggerEvent;
