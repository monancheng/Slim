    ...
    public class DoozyUIRedundancyCheck
    {
        ...
        static DoozyUIRedundancyCheck()
        {
            ...
            EditorApplication.hierarchyWindowChanged +=
                OnHierarchyWindowChanged; //we use this to run the code on scene change (in the editor); this is useful if the user (dev) loads a new scene and we need to check our component's integrity
        }

        private static void OnHierarchyWindowChanged()
        {
            if (string.IsNullOrEmpty(currentScene)
            ) //we just started Unity so we do not run this because the EditorApplication.update will handle it
                UpdateCurrentScene();

#if (UNITY_5_1 == false && UNITY_5_2 == false)
            if (currentScene != SceneManager.GetActiveScene().name) //a scene change has happened
#else
        ...
        private static void RunOnce()
        {
            ...
            EditorApplication.update -=
                RunOnce; //we stop listening for editor update (it will be run again only after the editor has recomplied the assemblies)

            StartTheRedundancyCheck();
        }

        private static void StartTheRedundancyCheck()
        {
        ...
        private static void UIElementsRedundancyCheck()
        {
        ...
        private static void UIButtonsRedundancyCheck()
        {
        ...
        {
            var uiElements = Object.FindObjectsOfType<UIElement>(); //get all the UIElements in the current scene
        ...
        private static string CheckIfElementSoundIsInDatabase(string s)
        {
            if (string.IsNullOrEmpty(s)) //check if the string is null or empty
                s = UIManager.DEFAULT_SOUND_NAME; //set default value
            else if (UIManager.GetIndexForElementSound(s) == -1
            ) //the sound name does not exist in the database it got corrupted; we reset it to the default value
                s = UIManager.DEFAULT_SOUND_NAME; //set default value
            return s;
        }
        ...
        public static void UIElementRedundancyCheck(UIElement uiElement, bool debugThis = false)
        {
            ...
            if (uiElement.GetComponent<Canvas>() == null)
            {
                ...
                Debug.Log("[DoozyUI] [RedundancyCheck] [" + uiElement.name +
                          "] The UIElement didn't have a <Canvas> component attached and it was just added automatically. The missing <Canvas> might cause visibility issues.");
            }
            ...
            if (uiElement.GetComponent<GraphicRaycaster>() == null)
            {
                ...
                Debug.Log("[DoozyUI] [RedundancyCheck] [" + uiElement.name +
                          "] The UIElement didn't have a <GraphicRaycaster> component attached and it was just added automatically. The missing <GraphicRaycaster> might make this UIElement not be able to receive clicks/touches.");
            }
            ...
            else if (UIManager.GetIndexForElementName(uiElement.elementName) == -1
            ) //the backup name does not exist in the database it got corrupted; we reset it to the default value
                uiElement.elementName = UIManager.DEFAULT_ELEMENT_NAME;
            ...
            if (uiElement.moveInSoundAtStart.Equals(uiElement.moveIn.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveIn.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveInSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveInSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveInSoundAtStart)]; //we update the reference from the database
                    }
                    ...
                    uiElement.moveInSoundAtStart =
                        uiElement.moveIn.soundAtStartReference
                            .soundName; //we update the backup sound to the referenced sound
            ...
            if (uiElement.moveInSoundAtFinish.Equals(uiElement.moveIn.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveIn.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveInSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveInSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveInSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.moveInSoundAtFinish =
                        uiElement.moveIn.soundAtFinishReference
            ...
            if (uiElement.rotationInSoundAtStart.Equals(uiElement.rotationIn.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationIn.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationInSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationInSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationInSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationInSoundAtStart =
                        uiElement.rotationIn.soundAtStartReference
            ...
            if (uiElement.rotationInSoundAtFinish.Equals(uiElement.rotationIn.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationIn.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationInSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationInSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationInSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationInSoundAtFinish =
                        uiElement.rotationIn.soundAtFinishReference
            ...
            if (uiElement.scaleInSoundAtStart.Equals(uiElement.scaleIn.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleIn.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleInSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleInSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleInSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleInSoundAtStart =
                        uiElement.scaleIn.soundAtStartReference
            ...
            if (uiElement.scaleInSoundAtFinish.Equals(uiElement.scaleIn.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleIn.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleInSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleInSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleInSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleInSoundAtFinish =
                        uiElement.scaleIn.soundAtFinishReference
            ...
            if (uiElement.fadeInSoundAtStart.Equals(uiElement.fadeIn.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeIn.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeInSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeInSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeIn.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeInSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeInSoundAtStart =
                        uiElement.fadeIn.soundAtStartReference
            ...
            if (uiElement.fadeInSoundAtFinish.Equals(uiElement.fadeIn.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeIn.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeInSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeInSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeIn.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeInSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeInSoundAtFinish =
                        uiElement.fadeIn.soundAtFinishReference
            ...
            if (uiElement.moveLoopSoundAtStart.Equals(uiElement.moveLoop.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveLoop.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveLoopSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveLoopSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveLoopSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.moveLoopSoundAtStart =
                        uiElement.moveLoop.soundAtStartReference
            ...
            if (uiElement.moveLoopSoundAtFinish.Equals(uiElement.moveLoop.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveLoop.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveLoopSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveLoopSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveLoopSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.moveLoopSoundAtFinish =
                        uiElement.moveLoop.soundAtFinishReference
            ...
            if (uiElement.rotationLoopSoundAtStart.Equals(uiElement.rotationLoop.soundAtStartReference.soundName) ==
                false) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationLoop.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationLoopSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationLoopSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationLoopSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationLoopSoundAtStart =
                        uiElement.rotationLoop.soundAtStartReference
            ...
            if (uiElement.rotationLoopSoundAtFinish.Equals(uiElement.rotationLoop.soundAtFinishReference.soundName) ==
                false) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationLoop.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationLoopSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationLoopSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationLoopSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationLoopSoundAtFinish =
                        uiElement.rotationLoop.soundAtFinishReference
            ...
            if (uiElement.scaleLoopSoundAtStart.Equals(uiElement.scaleLoop.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleLoop.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleLoopSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleLoopSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleLoopSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleLoopSoundAtStart =
                        uiElement.scaleLoop.soundAtStartReference
            ...
            if (uiElement.scaleLoopSoundAtFinish.Equals(uiElement.scaleLoop.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleLoop.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleLoopSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleLoopSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleLoopSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleLoopSoundAtFinish =
                        uiElement.scaleLoop.soundAtFinishReference
            ...
            if (uiElement.fadeLoopSoundAtStart.Equals(uiElement.fadeLoop.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeLoop.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeLoopSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeLoopSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeLoop.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeLoopSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeLoopSoundAtStart =
                        uiElement.fadeLoop.soundAtStartReference
            ...
            if (uiElement.fadeLoopSoundAtFinish.Equals(uiElement.fadeLoop.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeLoop.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeLoopSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeLoopSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeLoop.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeLoopSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeLoopSoundAtFinish =
                        uiElement.fadeLoop.soundAtFinishReference
            ...
            if (uiElement.moveOutSoundAtStart.Equals(uiElement.moveOut.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveOut.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveOutSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveOutSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveOutSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.moveOutSoundAtStart =
                        uiElement.moveOut.soundAtStartReference
            ...
            if (uiElement.moveOutSoundAtFinish.Equals(uiElement.moveOut.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.moveOut.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.moveOutSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.moveOutSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.moveOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.moveOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .moveOutSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.moveOutSoundAtFinish =
                        uiElement.moveOut.soundAtFinishReference
            ...
            if (uiElement.rotationOutSoundAtStart.Equals(uiElement.rotationOut.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationOut.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationOutSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationOutSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationOutSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationOutSoundAtStart =
                        uiElement.rotationOut.soundAtStartReference
            ...
            if (uiElement.rotationOutSoundAtFinish.Equals(uiElement.rotationOut.soundAtFinishReference.soundName) ==
                false) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.rotationOut.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.rotationOutSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.rotationOutSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.rotationOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.rotationOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .rotationOutSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.rotationOutSoundAtFinish =
                        uiElement.rotationOut.soundAtFinishReference
            ...
            if (uiElement.scaleOutSoundAtStart.Equals(uiElement.scaleOut.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleOut.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleOutSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleOutSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleOutSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleOutSoundAtStart =
                        uiElement.scaleOut.soundAtStartReference
            ...
            if (uiElement.scaleOutSoundAtFinish.Equals(uiElement.scaleOut.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.scaleOut.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.scaleOutSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.scaleOutSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.scaleOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.scaleOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .scaleOutSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.scaleOutSoundAtFinish =
                        uiElement.scaleOut.soundAtFinishReference
            ...
            if (uiElement.fadeOutSoundAtStart.Equals(uiElement.fadeOut.soundAtStartReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeOut.soundAtStartReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeOutSoundAtStart) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeOutSoundAtStart =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeOut.soundAtStartReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeOutSoundAtStart)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeOutSoundAtStart =
                        uiElement.fadeOut.soundAtStartReference
            ...
            if (uiElement.fadeOutSoundAtFinish.Equals(uiElement.fadeOut.soundAtFinishReference.soundName) == false
            ) //the referenced sound is not the same as the backup name, we check if a rollbakck is needed
                if (UIManager.GetIndexForElementSound(uiElement.fadeOut.soundAtFinishReference.soundName) == -1
                ) //if TRUE, the referenced sound does not exist in the database (we might have an error); we need to check that the backup sound is not corrupted as well
                    if (UIManager.GetIndexForElementSound(uiElement.fadeOutSoundAtFinish) == -1
                    ) //the backup sound does not exist in the database or it is the default sound; we perform the rollback
                    {
                        uiElement.fadeOutSoundAtFinish =
                            UIManager.DEFAULT_SOUND_NAME; //we set the backup sound as the default sound
                        uiElement.fadeOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(UIManager
                                    .DEFAULT_SOUND_NAME)]; //we reference the default sound from the database
                    }
                    else //the referenced sound does not exit (we had a serialization error), but the backup is fine (we found it in the database) --> we update the reference
                    {
                        uiElement.fadeOut.soundAtFinishReference =
                            UIManager.GetDoozyUIData.elementSounds[
                                UIManager.GetIndexForElementSound(uiElement
                                    .fadeOutSoundAtFinish)]; //we update the reference from the database
                    }
                else //the referenced sound exists in the database, so we update the backup (this is for upgrade purposes, so that we get the proper backup sound)
                    uiElement.fadeOutSoundAtFinish =
                        uiElement.fadeOut.soundAtFinishReference
        ...
        {
            var uiButtons = Object.FindObjectsOfType<UIButton>(); //get all the UIButtons in the current scene
        ...
        public static void UIButtonRedundancyCheck(UIButton uiButton, bool debugThis = false)
        {
            ...
            else if (UIManager.GetIndexForButtonName(uiButton.buttonName) == -1
            ) //the backup name does not exist in the database it got corrupted; we reset it to the default value
                uiButton.buttonName = UIManager.DEFAULT_BUTTON_NAME;
            ...
            else if (UIManager.GetIndexForButtonSound(uiButton.onClickSound) == -1
            ) //the backup sound name does not exist in the database it got corrupted; we reset it to the default value
                uiButton.onClickSound = UIManager.DEFAULT_SOUND_NAME;
