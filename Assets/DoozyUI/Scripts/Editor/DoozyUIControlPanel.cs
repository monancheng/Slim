...
namespace DoozyUI
{
    ...
    public class DoozyUIControlPanel : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowMessageAddDoozy, GUIStyle.none, GUILayout.Height(535),
                    GUILayout.Width(600)))
                {
        ...
        private void UpdateHeader()
        {
            ...
            switch (nextPage)
            {
                ...
                case Page.ElementNames:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderElementNames, GUIStyle.none,
                        GUILayout.Height(32), GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;

                case Page.ElementSounds:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderElementSounds, GUIStyle.none,
                        GUILayout.Height(32), GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;

                case Page.ButtonNames:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderButtonNames, GUIStyle.none,
                        GUILayout.Height(32), GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;

                case Page.ButtonSounds:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderButtonSounds, GUIStyle.none,
                        GUILayout.Height(32), GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;

                case Page.QuickHelp:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderQuickHelp, GUIStyle.none, GUILayout.Height(32),
                        GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;

                case Page.VideoTutorials:
                    if (GUILayout.Button(DoozyUIResources.WindowSubheaderVideoTutorials, GUIStyle.none,
                        GUILayout.Height(32), GUILayout.Width(600)))
                        GotoPage(Page.Main);
                    break;
            }
        ...
        private void ShowYouTubePlayButton(int spaceBetweenEachItem, int spaceBeforeEachIntem, string label,
            string link)
        {
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(spaceBeforeEachIntem);
            if (GUILayout.Button(DoozyUIResources.WindowButtonPlay, GUIStyle.none, GUILayout.Width(64),
                GUILayout.Height(24)))
                Application.OpenURL(link);
            GUILayout.Space(4);
            EditorGUILayout.LabelField(label,
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.White, TextAnchor.MiddleLeft,
                    FontStyle.BoldAndItalic, 12), GUILayout.Height(22));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(spaceBetweenEachItem);
        }

        private void ShowTitle(int spaceBetweenEachItem, string title)
        {
            GUILayout.Space(spaceBetweenEachItem * 3);
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField(title,
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.Orange, TextAnchor.MiddleCenter,
                    FontStyle.BoldAndItalic, 14), GUILayout.Width(300));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            GUILayout.Space(spaceBetweenEachItem);
        }
        ...
        private UIElement[] uiElementsInTheScene;
        private UIButton[] uiButtonsInTheScene;
        ...
        private int elementNamesDeleteIndex = -1;
        private int elementSoundsDeleteIndex = -1;
        private int buttonNamesDeleteIndex = -1;
        private int buttonSoundsDeleteIndex = -1;

        private bool elementNamesNew;
        private bool elementSoundsNew;
        private bool buttonNamesNew;
        private bool buttonSoundsNew;

        private string elementNamesNewString = string.Empty;
        private string elementSoundsNewString = string.Empty;
        private string buttonNamesNewString = string.Empty;
        private string buttonSoundsNewString = string.Empty;

        private int elementNamesRenameIndex = -1;
        private int elementSoundsRenameIndex = -1;
        private int buttonNamesRenameIndex = -1;
        private int buttonSoundsRenameIndex = -1;

        private string elementNamesRenameString = string.Empty;
        private string elementSoundsRenameString = string.Empty;
        private string buttonNamesRenameString = string.Empty;
        private string buttonSoundsRenameString = string.Empty;

        private int elementNamesFinderIndex;
        private int elementSoundsFinderIndex;
        private int buttonNamesFinderIndex;
        private int buttonSoundsFinderIndex;
        ...
        private List<UIElement> elementNamesFilterList;
        private List<UIElement> elementSoundsFilterList;
        private List<UIButton> buttonNamesFilterList;
        private List<UIButton> buttonSoundsFilterList;
        ...
        private readonly int numberOfFilteredButtonsPerRow = 3;

        private bool resetElementNames;
        private bool resetElementSounds;
        private bool resetButtonNames;
        private bool resetButtonSounds;
        ...
        /// </summary>
        private void UpdateUIElementsArray()
        {
        ...
        /// <returns></returns>
        private List<UIElement> GetAllTheUIElementWithName(string eName)
        {
            ...
            {
                for (var i = 0; i < uiElementsInTheScene.Length; i++)
                    if (uiElementsInTheScene[i].linkedToNotification == false &&
                        eName.Equals(uiElementsInTheScene[i].elementName))
        ...
        /// <returns></returns>
        private List<UIElement> GetAllTheUIElementWithSound(string eSound)
        {
            var tempList = new List<UIElement>();
        ...
        /// <param name="eName"></param>
        /// <returns></returns>
        private int GetUIElementNameCount(string eName)
        {
            var count = 0;
            if (uiElementsInTheScene != null)
                for (var i = 0; i < uiElementsInTheScene.Length; i++)
        ...
        /// <param name="eSound"></param>
        /// <returns></returns>
        private int GetUIElementSoundCount(string eSound)
        {
            var count = 0;
            if (uiElementsInTheScene != null)
                for (var i = 0; i < uiElementsInTheScene.Length; i++)
        ...
        /// </summary>
        private void UpdateUIButtonsArray()
        {
        ...
        /// <returns></returns>
        private List<UIButton> GetAllTheUIButtonWithName(string bName)
        {
            ...
            {
                for (var i = 0; i < uiButtonsInTheScene.Length; i++)
        ...
        /// <returns></returns>
        private List<UIButton> GetAllTheUIButtonWithSound(string bSound)
        {
            var tempList = new List<UIButton>();
        ...
        /// <param name="bName"></param>
        /// <returns></returns>
        private int GetUIButtonsNameCount(string bName)
        {
            var count = 0;
            if (uiButtonsInTheScene != null)
                for (var i = 0; i < uiButtonsInTheScene.Length; i++)
        ...
        /// <param name="bSound"></param>
        /// <returns></returns>
        private int GetUIButtonsSoundCount(string bSound)
        {
            var count = 0;
            if (uiButtonsInTheScene != null)
                for (var i = 0; i < uiButtonsInTheScene.Length; i++)
        ...
        private void ShowMainPage()
        {
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonMADisabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonTMPDisabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonEBTDisabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonPMDisabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonOMDisabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
...
#else
            if (GUILayout.Button(DoozyUIResources.WindowButtonNavEnabled, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonElementNames, GUIStyle.none, GUILayout.Height(48),
                    GUILayout.Width(134)))
                    GotoPage(Page.ElementNames);
                GUILayout.Space(4);
                if (GUILayout.Button(DoozyUIResources.WindowButtonResetRed, GUIStyle.none, GUILayout.Height(24),
                    GUILayout.Width(134)))
                    resetElementNames = true;
            }
            else
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonResetElementNames, GUIStyle.none,
                    GUILayout.Height(48), GUILayout.Width(134)))
                {
                    UIManager.ResetDoozyUIDataElementNames();
                    resetElementNames = false;
                }
                GUILayout.Space(4);
                if (GUILayout.Button(DoozyUIResources.WindowButtonCancelGreen, GUIStyle.none, GUILayout.Height(24),
                    GUILayout.Width(134)))
                    resetElementNames = false;
            }
            EditorGUILayout.EndVertical();
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonElementSounds, GUIStyle.none, GUILayout.Height(48),
                    GUILayout.Width(134)))
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonResetElementsounds, GUIStyle.none,
                    GUILayout.Height(48), GUILayout.Width(134)))
                {
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonButtonNames, GUIStyle.none, GUILayout.Height(48),
                    GUILayout.Width(134)))
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonResetButtonNames, GUIStyle.none, GUILayout.Height(48),
                    GUILayout.Width(134)))
                {
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonButtonSounds, GUIStyle.none, GUILayout.Height(48),
                    GUILayout.Width(134)))
            ...
            {
                if (GUILayout.Button(DoozyUIResources.WindowButtonResetButtonSounds, GUIStyle.none,
                    GUILayout.Height(48), GUILayout.Width(134)))
                {
            ...
            if (GUILayout.Button(DoozyUIResources.WindowButtonQuickHelp, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
            ...
            if (GUILayout.Button(DoozyUIResources.WindowButtonVideoTutorials, GUIStyle.none, GUILayout.Height(48),
                GUILayout.Width(134)))
            ...
            */
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("DoozyUI " + UIManager.VERSION,
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.Doozy, TextAnchor.MiddleCenter, FontStyle.Italic,
                    12), GUILayout.Width(600));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField(UIManager.COPYRIGHT,
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.Doozy, TextAnchor.MiddleCenter,
                    FontStyle.BoldAndItalic, 8), GUILayout.Width(600));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.EndVertical();
        ...
        private void ShowElementNames()
        {
            ...
            else
            {
                ...
                elementNamesNewString =
                    EditorGUILayout.TextField(elementNamesNewString, GUILayout.Height(20), GUILayout.Width(276));
                if (GUILayout.Button("create", GUILayout.Height(18), GUILayout.Width(56))
                ) //we add a rename button for each list entry
                {
                ...
                ) //we add a delete button for each list entry
                {
                    elementNamesNewString = string.Empty;
                    elementNamesNew = false;
                }
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("Used",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(50));
            EditorGUILayout.LabelField("Index",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(70));
            EditorGUILayout.LabelField("Element Name",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(320));
            EditorGUILayout.LabelField("Options",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(130));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            ...
            elementNamesScrollPosition =
                EditorGUILayout.BeginScrollView(elementNamesScrollPosition, GUILayout.Height(345));
            for (var i = 0; i < elementNames.Length; i++) //show the all the elementNames entries
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                //Used
                ...
                if (GUILayout.Button(GetUIElementNameCount(elementNames[i]).ToString(), GUILayout.Height(18),
                    GUILayout.Width(60)))
                ...
                EditorGUILayout.LabelField(i.ToString(),
                    DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                        FontStyle.Normal), GUILayout.Height(18), GUILayout.Width(60));
                if (elementNames[i].Equals(UIManager.DEFAULT_ELEMENT_NAME)) //no options for the default element name
                {
                    //Element Name
                    EditorGUILayout.LabelField(elementNames[i],
                        DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleLeft,
                            FontStyle.Normal), GUILayout.Height(18), GUILayout.Width(330));
                    //Options
                    GUILayout.Space(120);
                }
                else
                {
                    ...
                    else if (elementNamesRenameIndex != -1) //RENAME
                    {
                        if (i == elementNamesRenameIndex) //this is the index we want to rename
                        {
                            ...
                            elementNamesRenameString = EditorGUILayout.TextField(elementNamesRenameString,
                                GUILayout.Height(18), GUILayout.Width(330));
                            //Options
                            if (GUILayout.Button("rename", GUILayout.Height(18), GUILayout.Width(56)))
                            {
                                UIManager.TrimStartAndEndSpaces(elementNamesRenameString);
                                if (elementNamesRenameString.Equals(string.Empty) ==
                                    false //we make sure the new name is not empty
                                    && elementNamesRenameString.Equals(UIManager.DEFAULT_ELEMENT_NAME) ==
                                    false //we check that is not the default name
                                    && UIManager.GetIndexForElementName(elementNamesRenameString) == -1
                                ) //we make sure there are no duplicates
                                {
                                    UIManager.RenameElementName(elementNamesRenameIndex,
                                        elementNamesRenameString); //we rename the element name
                                    elementNames =
                                        UIManager.GetElementNames(); //we update the string array for this list
                                    DoozyUIRedundancyCheck.CheckAllTheUIElements();
                                }
                    ...
                    else if (elementNamesDeleteIndex != -1) //DELETE
                    {
                        if (i == elementNamesDeleteIndex) //this is the index we want to delete
                        {
                            //Delete ElementName
                            EditorGUILayout.LabelField(elementNames[i],
                                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightRed, TextAnchor.MiddleLeft,
                                    FontStyle.Bold), GUILayout.Height(18), GUILayout.Width(330));
                            //Options
                            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightRed);
                            if (GUILayout.Button("delete", GUILayout.Height(18), GUILayout.Width(56)))
                            {
                                ...
                                elementNamesFinderIndex =
                                    0; //we reset the finder filter in case we are deleting the currently selected entry
                                UpdateUIElementsArray();
                            }
                            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightGreen);
                            if (GUILayout.Button("cancel", GUILayout.Height(18), GUILayout.Width(56)))
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(30);
            EditorGUILayout.LabelField("SHOWING all the UIElements with the",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Bold, 10), GUILayout.Width(200));
            EditorGUILayout.LabelField(elementNames[elementNamesFinderIndex],
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 12), GUILayout.Width(260));
            EditorGUILayout.LabelField("Element Name",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleLeft, FontStyle.Bold,
                    10), GUILayout.Width(70));
            GUILayout.Space(30);
            EditorGUILayout.EndHorizontal();
            DoozyUIHelper.DrawTexture(DoozyUIResources.MiniBarLightGrey, 600f, 2f);
            DoozyUIHelper.VerticalSpace(2);
            elementNamesFilterScrollPosition =
                EditorGUILayout.BeginScrollView(elementNamesFilterScrollPosition, GUILayout.Height(85));
            ...
            if (elementNamesFilterList != null && elementNamesFilterList.Count > 0)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                for (var i = 0; i < elementNamesFilterList.Count; i++)
                {
                    if (GUILayout.Button(elementNamesFilterList[i].gameObject.name, GUILayout.Height(18),
                        GUILayout.Width(180)))
            ...
            else
            {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                EditorGUILayout.LabelField("There are no UIElements in the scene with the selected element name!",
                    DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                        FontStyle.Italic));
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
        ...
        private void ShowElementSounds()
        {
            DoozyUIHelper.ResetColors();
            ...
            {
                DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightGreen);
                elementSoundsNewString =
                    EditorGUILayout.TextField(elementSoundsNewString, GUILayout.Height(20), GUILayout.Width(276));
                if (GUILayout.Button("create", GUILayout.Height(18), GUILayout.Width(56))
                ) //we add a rename button for each list entry
                {
            ...
            EditorGUILayout.LabelField("Index",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(70));
            EditorGUILayout.LabelField("Element Sound",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(320));
            EditorGUILayout.LabelField("Options",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
            ...
            elementSoundsScrollPosition =
                EditorGUILayout.BeginScrollView(elementSoundsScrollPosition, GUILayout.Height(345));
            for (var i = 0; i < elementSounds.Length; i++) //show the all the elementSounds entries
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                //Used
                ...
                if (GUILayout.Button(GetUIElementSoundCount(elementSounds[i]).ToString(), GUILayout.Height(18),
                    GUILayout.Width(60)))
                ...
                {
                    //Element Sound
                    EditorGUILayout.LabelField(elementSounds[i],
                        DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleLeft,
                            FontStyle.Normal), GUILayout.Height(18), GUILayout.Width(330));
                    //Options
                    GUILayout.Space(120);
                ...
                else
                {
                    ...
                    else if (elementSoundsRenameIndex != -1) //RENAME
                    {
                        if (i == elementSoundsRenameIndex) //this is the index we want to rename
                        {
                            //Rename ElementName
                            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightOranage);
                            elementSoundsRenameString = EditorGUILayout.TextField(elementSoundsRenameString,
                                GUILayout.Height(18), GUILayout.Width(330));
                            //Options
                            if (GUILayout.Button("rename", GUILayout.Height(18), GUILayout.Width(56)))
                            {
                                UIManager.TrimStartAndEndSpaces(elementSoundsRenameString);
                                if (elementSoundsRenameString.Equals(string.Empty) ==
                                    false //we make sure the new name is not empty
                                    && elementSoundsRenameString.Equals(UIManager.DEFAULT_SOUND_NAME) ==
                                    false //we check that is not the default name
                                    && UIManager.GetIndexForElementSound(elementSoundsRenameString) == -1
                                ) //we make sure there are no duplicates
                                {
                                    UIManager.RenameElementSound(elementSoundsRenameIndex,
                                        elementSoundsRenameString); //we rename the element name
                                    elementSounds =
                                        UIManager.GetElementSounds(); //we update the string array for this list
                                    DoozyUIRedundancyCheck.CheckAllTheUIElements();
                                }
            ...
            EditorGUILayout.LabelField("SHOWING all the UIElements with the",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Bold, 10), GUILayout.Width(200));
            EditorGUILayout.LabelField(elementSounds[elementSoundsFinderIndex],
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 12), GUILayout.Width(260));
            EditorGUILayout.LabelField("Element Sound",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleLeft, FontStyle.Bold,
            ...
            DoozyUIHelper.DrawTexture(DoozyUIResources.MiniBarLightBlue, 600f, 2f);
            DoozyUIHelper.VerticalSpace(2);
            elementSoundsFilterScrollPosition =
                EditorGUILayout.BeginScrollView(elementSoundsFilterScrollPosition, GUILayout.Height(85));
            ...
            if (elementSoundsFilterList != null && elementSoundsFilterList.Count > 0)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                for (var i = 0; i < elementSoundsFilterList.Count; i++)
                {
                    if (GUILayout.Button(elementSoundsFilterList[i].gameObject.name, GUILayout.Height(18),
                        GUILayout.Width(180)))
            ...
            else
            {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                EditorGUILayout.LabelField("There are no gameObjects in the scene with the selected element sound!",
                    DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                        FontStyle.Italic));
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
        ...
        private void ShowButtonNames()
        {
            ...
            {
                DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightGreen);
                buttonNamesNewString =
                    EditorGUILayout.TextField(buttonNamesNewString, GUILayout.Height(20), GUILayout.Width(276));
                if (GUILayout.Button("create", GUILayout.Height(18), GUILayout.Width(56))
                ) //we add a rename button for each list entry
                {
            ...
            buttonNamesScrollPosition =
                EditorGUILayout.BeginScrollView(buttonNamesScrollPosition, GUILayout.Height(345));
            for (var i = 0; i < buttonNames.Length; i++) //show the all the buttonNames entries
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                //Used
                ...
                if (GUILayout.Button(GetUIButtonsNameCount(buttonNames[i]).ToString(), GUILayout.Height(18),
                    GUILayout.Width(60)))
                ...
                {
                    //Button Name
                    EditorGUILayout.LabelField(buttonNames[i],
                        DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleLeft,
                            FontStyle.Normal), GUILayout.Height(18), GUILayout.Width(330));
                    //Options
                    GUILayout.Space(120);
                ...
                else
                {
                    ...
                    else if (buttonNamesRenameIndex != -1) //RENAME
                    {
                        if (i == buttonNamesRenameIndex) //this is the index we want to rename
                        {
                            //Rename ButtonName
                            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightOranage);
                            buttonNamesRenameString = EditorGUILayout.TextField(buttonNamesRenameString,
                                GUILayout.Height(18), GUILayout.Width(330));
                            //Options
                            if (GUILayout.Button("rename", GUILayout.Height(18), GUILayout.Width(56)))
                            {
                                UIManager.TrimStartAndEndSpaces(buttonNamesRenameString);
                                if (buttonNamesRenameString.Equals(string.Empty) ==
                                    false //we make sure the new name is not empty
                                    && buttonNamesRenameString.Equals(UIManager.DEFAULT_BUTTON_NAME) ==
                                    false //we check that is not the default name
                                    && buttonNamesRenameString.Equals("Back") == false
                                    && UIManager.GetIndexForButtonName(buttonNamesRenameString) == -1
                                ) //we make sure there are no duplicates
                                {
                                    UIManager.RenameButtonName(buttonNamesRenameIndex,
                                        buttonNamesRenameString); //we rename the button name
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(30);
            EditorGUILayout.LabelField("SHOWING all the UIButtons with the",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Bold, 10), GUILayout.Width(200));
            EditorGUILayout.LabelField(buttonNames[buttonNamesFinderIndex],
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 12), GUILayout.Width(260));
            EditorGUILayout.LabelField("Button Name",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleLeft, FontStyle.Bold,
            ...
            DoozyUIHelper.DrawTexture(DoozyUIResources.MiniBarLightBlue, 600f, 2f);
            DoozyUIHelper.VerticalSpace(2);
            buttonNamesFilterScrollPosition =
                EditorGUILayout.BeginScrollView(buttonNamesFilterScrollPosition, GUILayout.Height(85));
            ...
            if (buttonNamesFilterList != null && buttonNamesFilterList.Count > 0)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                for (var i = 0; i < buttonNamesFilterList.Count; i++)
                {
                    if (GUILayout.Button(buttonNamesFilterList[i].gameObject.name, GUILayout.Height(18),
                        GUILayout.Width(180)))
            ...
            else
            {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                EditorGUILayout.LabelField("There are no gameObjects in the scene with the selected button name!",
                    DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                        FontStyle.Italic));
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
        ...
        private void ShowButtonSounds()
        {
            ...
            {
                DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightGreen);
                buttonSoundsNewString =
                    EditorGUILayout.TextField(buttonSoundsNewString, GUILayout.Height(20), GUILayout.Width(276));
                if (GUILayout.Button("create", GUILayout.Height(18), GUILayout.Width(56))
                ) //we add a rename button for each list entry
                {
            ...
            EditorGUILayout.LabelField("Index",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(70));
            EditorGUILayout.LabelField("Button Sound",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
                    FontStyle.Bold), GUILayout.Height(20), GUILayout.Width(320));
            EditorGUILayout.LabelField("Options",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleCenter,
            ...
            buttonSoundsScrollPosition =
                EditorGUILayout.BeginScrollView(buttonSoundsScrollPosition, GUILayout.Height(345));
            for (var i = 0; i < buttonSounds.Length; i++) //show the all the buttonSounds entries
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                //Used
                ...
                if (GUILayout.Button(GetUIButtonsSoundCount(buttonSounds[i]).ToString(), GUILayout.Height(18),
                    GUILayout.Width(60)))
                ...
                {
                    //Button Sound
                    EditorGUILayout.LabelField(buttonSounds[i],
                        DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightGrey, TextAnchor.MiddleLeft,
                            FontStyle.Normal), GUILayout.Height(18), GUILayout.Width(330));
                    //Options
                    GUILayout.Space(120);
                ...
                else
                {
                    ...
                    else if (buttonSoundsRenameIndex != -1) //RENAME
                    {
                        if (i == buttonSoundsRenameIndex) //this is the index we want to rename
                        {
                            //Rename ButtonSound
                            DoozyUIHelper.SetZoneColor(DoozyUIHelper.DoozyColor.LightOranage);
                            buttonSoundsRenameString = EditorGUILayout.TextField(buttonSoundsRenameString,
                                GUILayout.Height(18), GUILayout.Width(330));
                            //Options
                            if (GUILayout.Button("rename", GUILayout.Height(18), GUILayout.Width(56)))
                            {
                                UIManager.TrimStartAndEndSpaces(buttonSoundsRenameString);
                                if (buttonSoundsRenameString.Equals(string.Empty) ==
                                    false //we make sure the new name is not empty
                                    && buttonSoundsRenameString.Equals(UIManager.DEFAULT_SOUND_NAME) ==
                                    false //we check that is not the default name
                                    && UIManager.GetIndexForButtonName(buttonSoundsRenameString) == -1
                                ) //we make sure there are no duplicates
                                {
                                    UIManager.RenameButtonSound(buttonSoundsRenameIndex,
                                        buttonSoundsRenameString); //we rename the button sound
                                    buttonSounds =
                                        UIManager.GetButtonSounds(); //we update the string array for this list
                                    DoozyUIRedundancyCheck.CheckAllTheUIButtons();
                                }
            ...
            EditorGUILayout.LabelField("SHOWING all the UIButtons with the",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Bold, 10), GUILayout.Width(200));
            EditorGUILayout.LabelField(buttonSounds[buttonSoundsFinderIndex],
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 12), GUILayout.Width(260));
            EditorGUILayout.LabelField("Button Sound",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleLeft, FontStyle.Bold,
            ...
            DoozyUIHelper.DrawTexture(DoozyUIResources.MiniBarLightBlue, 600f, 2f);
            DoozyUIHelper.VerticalSpace(2);
            buttonSoundsFilterScrollPosition =
                EditorGUILayout.BeginScrollView(buttonSoundsFilterScrollPosition, GUILayout.Height(85));
            ...
            if (buttonSoundsFilterList != null && buttonSoundsFilterList.Count > 0)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                for (var i = 0; i < buttonSoundsFilterList.Count; i++)
                {
                    if (GUILayout.Button(buttonSoundsFilterList[i].gameObject.name, GUILayout.Height(18),
                        GUILayout.Width(180)))
            ...
            else
            {
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                EditorGUILayout.LabelField("There are no gameObjects in the scene with the selected button sound!",
                    DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.LightBlue, TextAnchor.MiddleCenter,
                        FontStyle.Italic));
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
        ...
        private void ShowQuickHelp()
        {
            ...
            var spaceBeforeEachIntem = 128;
            var spaceBetweenEachItem = 4;

            #region Disclaimer

            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("You will find up to date informations by accesing the online documentation",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.Orange, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 9), GUILayout.Width(300));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            if (GUILayout.Button(DoozyUIResources.WindowButtonOpenDocumentation, GUIStyle.none, GUILayout.Width(192),
                GUILayout.Height(48)))
                Application.OpenURL(
                    "https://dl.dropboxusercontent.com/u/82405904/DoozyUI/Documentation/DoozyUI%202.6%20-%20Documentation.pdf");
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            ...
            quickHelpScrollPosition =
                EditorGUILayout.BeginScrollView(quickHelpScrollPosition, GUILayout.Height(401), GUILayout.Width(590));

            ShowTitle(spaceBetweenEachItem, "About the DoozyUI components");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "The Control Panel",
                "https://www.youtube.com/watch?v=7q1rN-mIT88");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Manager",
                "https://www.youtube.com/watch?v=pjRwJeVQxgM");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Button",
                "https://www.youtube.com/watch?v=zJwGyFLh5_0");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Element",
                "https://www.youtube.com/watch?v=CEd_gQsh-p4");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Trigger",
                "https://www.youtube.com/watch?v=omQCWiMI2tg");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Effect",
                "https://www.youtube.com/watch?v=WygtDgmbVZQ");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "UI Notification",
                "https://www.youtube.com/watch?v=WlBcFz7CNmE");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "Scene Loader",
                "https://www.youtube.com/watch?v=QE24nljfqpk");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "Playmaker Event Dispatcher",
                "https://www.youtube.com/watch?v=h_Z7HUPbZx4");

            GUILayout.Space(spaceBetweenEachItem);

            ShowTitle(spaceBetweenEachItem, "3rd Party Plugins");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to enable support for MasterAudio",
                "https://www.youtube.com/watch?v=IxE4krN5-xo");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to enable support for TextMeshPro",
                "https://www.youtube.com/watch?v=Cwlf4L0chvU");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem,
                "How to enable support for EnergyBarToolkit", "https://www.youtube.com/watch?v=53FiqbrmoXo");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to enable support for Playmaker",
                "https://www.youtube.com/watch?v=lqJIRQXa7q0");

            GUILayout.Space(spaceBetweenEachItem);

            ShowTitle(spaceBetweenEachItem, "The Navigation System");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to disable the Navigation System",
                "https://www.youtube.com/watch?v=Afcq4hvT5OE");

            GUILayout.Space(spaceBetweenEachItem);

            ShowTitle(spaceBetweenEachItem, "The Orientation Manager");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to use the Orientation Manager",
                "https://youtu.be/XBGzmxqWA2c");
        ...
        private void ShowVideoTutorials()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("You will find more tutorials on our YouTube Channel",
                DoozyUIHelper.CreateTextStyle(DoozyUIHelper.DoozyColor.Orange, TextAnchor.MiddleCenter,
                    FontStyle.Italic, 9), GUILayout.Width(300));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            if (GUILayout.Button(DoozyUIResources.WindowButtonYoutube, GUIStyle.none, GUILayout.Width(136),
                GUILayout.Height(64)))
            ...
            videoTutorialsScrollPosition = EditorGUILayout.BeginScrollView(videoTutorialsScrollPosition,
                GUILayout.Height(385), GUILayout.Width(590));

            ShowTitle(spaceBetweenEachItem, "Release Notes");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.1 - New Features",
                "https://www.youtube.com/watch?v=ItrBJPtdsA4");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.2 - New Features",
                "https://youtu.be/DI7bSHSA2A8");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.3 - New Features",
                "https://youtu.be/fphA5T7SlHs");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.4 - New Features",
                "https://youtu.be/Mg_8PQ8ZlpI");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.5 - New Features",
                "https://youtu.be/wjBiONqijF4");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "DoozyUI 2.6 - New Features",
                "https://youtu.be/sAcLyHRNKi0");

            GUILayout.Space(spaceBetweenEachItem);

            ShowTitle(spaceBetweenEachItem, "Design Techniques");
            GUILayout.Space(spaceBetweenEachItem);
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to create a Game UI with Playmaker",
                "https://youtu.be/n_SdleiC10s");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How To create a Simple UI",
                "https://youtu.be/x5vubVxAv8I");
            ShowYouTubePlayButton(spaceBetweenEachItem, spaceBeforeEachIntem, "How to create a Parchment styled UI",
                "https://www.youtube.com/watch?v=quruI9a85vg");
