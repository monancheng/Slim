...
namespace DoozyUI
{
    ...
    public class Message
    {
        ...
        private static void RegisterListener(string messageName, Delegate callback)
        {
            if (callback == null)
            {
                Debug.LogError("Failed to add Message Listener because the given callback is null!");
                return;
            }
        ...
        private static void UnregisterListener(string messageName, Delegate callback)
        {
            ...
            var messageHandlers = handlers[messageName];
            var messageHandler = messageHandlers.Find(x => x.Method == callback.Method && x.Target == callback.Target);
        ...
        private static void SendMessage<T>(string messageName, T e) where T : Message
        {
            ...
            foreach (var messageHandler in messageHandlers)
            {
                ...
                {
                    var methodName = messageHandler.Method.Name;
